import { getReleaseNoteByVersion, insertReleaseNote, updateReleaseNoteContent } from './release-notes-db'
import { extractLatestVersionFromHtml, htmlToMarkdown, RELEASE_NOTES_FETCH_HEADERS as FETCH_HEADERS } from './release-notes-content'
...
// Scrape le forum "Release Notes Discussion" et retourne les versions récentes
// avec leur date de publication (timestamp embarqué dans le SSR). Liste vide en cas d'erreur.
export async function fetchForumTopics(): Promise<ForumTopic[]> {
  try {
    ...
    return parseTopicsFromForumHtml(html)
  } catch (error) {
...
// Date connue (liste statique) pour une version donnée
function getKnownVersionDate(version: string): string | null {
  return KNOWN_VERSIONS.find(v => v.version === version)?.date ?? null
}

// Résout la date de publication d'une version. La page officielle d'une release
// n'expose pas la date : on la récupère du forum, puis de la liste connue,
// et en dernier recours on retombe sur la date du jour.
export async function resolveReleaseDate(version: string): Promise<string> {
  const forumTopics = await fetchForumTopics()
  const forumMatch = forumTopics.find(t => t.version === version)
  if (forumMatch) return forumMatch.date
...
// fusionne avec les versions connues et insère en DB (sans créer de doublon).
// Utilisé par l'endpoint admin et par la tâche planifiée Nitro.
export async function syncReleaseNotes(): Promise<SyncResult> {
